...
export default function Detection({
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [isPaused, objectPresent, isLiveMode]);
  ...
  const activeTime =
    (isLiveMode && liveTime ? liveTime : simTime) ||
    new Date().toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
  ...
  // Dynamic Y ceiling according to current values
  const magnitudeCeiling = useMemo(() => {
    ...
    const step = paddedValue <= 50 ? 10 : paddedValue <= 150 ? 20 : 50;
  ...
  const yTicks = useMemo(() => {
    ...
    return Array.from({ length: tickCount + 1 }, (_, index) =>
      Math.round((magnitudeCeiling / tickCount) * index),
    );
  }, [magnitudeCeiling]);
  ...
  const pointsCoordinates = useMemo(() => {
    ...
    return activeAmplitudes.map((amp, index) => {
      ...
      return {
        x,
        y,
        magnitude: Number(amp) || 0,
        subcarrierIndex: index - 32,
        arrayBin: index,
      };
    });
  ...
  const amplitudeCurveColor = isLiveMode
    ? isDarkMode
      ? "#10b981"
      : "#059669"
    : isDarkMode
    ...
    <div className="flex-1 w-full px-4 md:px-8 py-6 flex flex-col gap-6 select-none bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-2xl p-4 md:p-6 shadow-xl relative flex flex-col">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
          ...
          <span className="text-xs text-slate-400">
            Standard chart axes activated below
          </span>
        </div>
        ...
        >
          <div
            className={`absolute top-4 left-4 z-10 border rounded-lg p-2.5 font-mono text-[10px] md:text-xs shadow-md ${
              isDarkMode
          ...
          {hoveredPoint && (
            <div
              className={`absolute bottom-12 right-4 z-20 border rounded-xl p-3 font-mono text-xs shadow-2xl animate-scaleUp pointer-events-none ${
                isDarkMode
                  ? "bg-slate-900/95 border-emerald-500 text-emerald-300"
                  : "bg-white/95 text-emerald-700 border-slate-300"
              }`}
            >
              ...
              <div>
                ...
                <strong className="text-slate-900 dark:text-white">
                  Index{" "}
                  {hoveredPoint.subcarrierIndex > 0
                    ? `+${hoveredPoint.subcarrierIndex}`
                    : hoveredPoint.subcarrierIndex}
                </strong>
              </div>
              <div>
                Magnitude :{" "}
                <strong className="text-slate-900 dark:text-white">
                  {hoveredPoint.magnitude.toFixed(2)}
                </strong>
              </div>
            </div>
          )}
          ...
          <svg viewBox="0 0 800 360" className="w-full h-full overflow-visible" preserveAspectRatio="none">
            ...
            />
            <rect
              x={paddingLeft + (59.25 / 63) * plotWidth}
              y={paddingTop}
              width={(3.75 / 63) * plotWidth}
              height={plotHeight}
              fill={guardBandFill}
            ...
            >
              ...
              <tspan className="text-emerald-600 dark:text-emerald-400 font-mono">
                LIVE TIMESTAMP: {activeTime}
              </tspan>
            </text>
      ...
      <div className="w-full flex flex-col items-center gap-3">
        ...
        >
          ...
          <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[10px] text-slate-500">
            64 Bins
          </span>
        </button>
          ...
          <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-xl font-mono text-xs animate-fadeIn">
            <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-100 dark:border-slate-800">
              <span className="font-bold text-slate-500 uppercase tracking-wider text-[11px]">
                Active Axis Matrix Packet #{activePktNum}
              </span>
              <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold">
                X = Subcarrier Index | Y = Calculated Mag sqrt(I^2+Q^2)
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-2 max-h-80 overflow-y-auto p-1">
              {activeAmplitudes.map((val, idx) => {
                ...
                if (sub === 0) {
                  bgPill = "bg-amber-50 dark:bg-amber-950/30 border-amber-300 text-amber-600 font-bold";
                }

                return (
                  <div
                    key={idx}
                    className={`p-2 rounded-xl border flex flex-col items-center justify-center text-[11px] ${bgPill}`}
                  >
                    <span className="text-[9px] text-slate-400 mb-0.5">
                      X: {sub > 0 ? `+${sub}` : sub}
                    </span>
                    <span className="font-mono font-bold">
                      {val ? Number(val).toFixed(1) : "0.0"}
                    </span>
                  </div>
                );
              })}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {isLiveMode && (
            <span className="text-xs text-amber-500 font-semibold">
              Click Manual Simulator above to unlock sliders
            </span>
          )}
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end gap-5 md:gap-10 w-full">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <label
                htmlFor="dist-detection"
                className="text-xs font-semibold text-slate-500 uppercase tracking-wider"
              >
                Transceiver Distance
              </label>
              <span className="text-sm font-mono font-bold">
                {Number(distance || 0).toFixed(1)} m
              </span>
            </div>
